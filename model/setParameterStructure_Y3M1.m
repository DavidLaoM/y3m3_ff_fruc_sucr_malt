%% INV
p.INV.VmINV=10.^x(177).*0.273; % mM/s (Bowski et al., 1971)
p.INV.KmINV=10.^x(178).*160; % mM (Bowski et al., 1971)

%% GLT
p.GLT.KeqGLT=1; % - (Stein, 1986)
p.GLT.KmGLTGLCi=10.^x(38).*1.1918; % mM (Teusink et al., 2000)
p.GLT.KmGLTGLCo=10.^x(38).*1.1918; % mM (Teusink et al., 2000)
p.GLT.VmGLT=0.2*10.^x(36).*3.67; % mM/s (Van Heerden et al., 2014)
p.GLT.KiGLTFRC=10.^x(166).*1.6667; % mM (Herwig et al., 2001)

%% FRT 
p.FRT.KeqFRT=1; % - (Stein, 1986)
p.FRT.KmFRTFRCo=10.^x(167).*6; % mM (Bisson & Fraenkel, 1983)
p.FRT.KmFRTFRCi=10.^x(167).*6; % mM (Bisson & Fraenkel, 1983)
p.FRT.VmFRT=0.2*10.^x(169).*3.67; % mM/s (value used for GLT)
p.FRT.KiFRTGLC=10.^x(170).*0.2778; % mM (Herwig et al., 2001)

%% MAT
p.MAT.KmMAT_ATP=10.^x(179).*0.7; % mM (Lecchi et al., 2005)
p.MAT.KmMAT_Maltec=10.^x(180).*4; % mM (Day et al., 2002; Mwesigye & Barford, 1996; Stambuk & de Araujo, 2001; Van Leeuwen et al., 1992)
p.MAT.VmMAT=10.^x(181).*0.39; % mM/s (Day et al., 2002; Stambuk & de Araujo, 2001)
p.MAT.nATP_nMaltec=10.^x(182).*1.0; % mol/mol (Weusthuis et al. 1993)

%% MALX2
p.MALX2.KmMALX2_Maltic=10.^x(183).*42.9; % mM (Matsusaka et al., 1977)
p.MALX2.VmMALX2=10.^x(184).*1.5776; % mM/s (unpublished black box model fitted to Maltec measurement, Koen Verhagen)

%% GLK 
% HXK1 Consensus
p.HXK1_Kadp=10.^x(28).*0.23; % mM (Teusink et al., 2000)
p.HXK1_Katp=10.^x(29).*0.15; % mM (Teusink et al., 2000)
p.HXK1_Keqglc=10.^x(30).*3800; % - (Teusink et al. 2000)
p.HXK1_Kg6p=10.^x(31).*30; % mM (Teusink et al., 2000)
p.HXK1_Kglc=10.^x(32).*0.08; % mM (Teusink et al., 2000)
p.HXK1_Kit6p=10.^x(33).*0.2; % mM (Blazquez et al., 1993; Van Eunen et al., 2012)
p.HXK1_kcatglc=0.2*10.^x(34).*4.75; % s-1 (Van Eunen et al., 2012)
p.HXK1_KiFRC=10.^x(171).*10^6; % Correct initial parameter value is unknown, therefore 10^6 (no inhibition) is put.

%% FRK
% HXK1 Consensus
p.HXK1_Keqfrc=10.^x(172).*1200; % eQuilibrator (website)
p.HXK1_Kf6p=10.^x(173).*30; % mM (value used for GLK)
p.HXK1_Kfrc=10.^x(174).*0.08.*1.5; % mM (Lobo & Maitra, 1977; Teusink et al., 2000)
p.HXK1_kcatfrc=0.2*10.^x(175).*4.75.*2; % s-1 (Lobo & Maitra, 1977; Teusink et al., 2000)
p.HXK1_KiGLC=10.^x(176).*10^6; % Correct initial parameter value is unknown, therefore 10^6 (no inhibition) is put.
